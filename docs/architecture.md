...
- **Process isolation**: `sussurro-llm-helper` owns the model and native runtime;
  the main process sends bounded synchronous prediction requests over stdio.
  This lets Whisper and Qwen use their separately vendored GPU backends without
  loading incompatible GGML runtimes into one process.
- **Prompt Engineering**:
